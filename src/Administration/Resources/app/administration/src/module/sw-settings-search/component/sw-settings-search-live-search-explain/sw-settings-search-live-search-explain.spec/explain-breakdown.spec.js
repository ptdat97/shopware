            ...
            extensions: {
                search: {
                    ...
                    matched_queries: {
                        ...
                        [JSON.stringify({ field: 'name', term: 'mediocre paper rippers', type: 'prefix', ranking: 700 })]:
                            3.1,
                    },
                },
            },
        ...
        const item = {
            ...
            extensions: {
                search: {
                    _score: 8048.4,
                    matched_queries: {
                        // both fire for the multi-word term: the boosted phrase clause (strong;
                        // its boost folds the field ranking, so the backend flags it `weighted`
                        // and its score arrives already weight-scaled) and a weaker raw prefix
                        // clause. The strong one must win the per-term dedup so the panel shows
                        // its real (high) score — kept as-is, NOT scaled by the ranking again.
                        [JSON.stringify({
                            field: 'name',
                            term: 'paper rippers',
                            type: 'phrase',
